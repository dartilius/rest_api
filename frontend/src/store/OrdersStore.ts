...
interface IOrdersStore {
  ...
  setTotalCountBg(count: number): void
  setTotalCountAd(count: number): void
}
...
class OrdersStore implements IOrdersStore {
  ...
  constructor() {
    makeObservable(this, {
      ...
      setTotalCountBg: action,
      setTotalCountAd: action,
    })
  }

  setTotalCountBg(count: number): void {
    this.totalCountBg = count
  }

  setTotalCountAd(count: number): void {
    this.totalCountAd = count
  }

  // Установка данных для Bg-заказов
  setDataBg(data: IDataBgResponse): void {
    this.dataBgResponse = data
    this.setTotalCountBg(data.count) // Используем action
  }
  // Установка данных для Ad-заказов
  setDataAd(data: IDataAdResponse): void {
    this.dataAdResponse = data
    this.setTotalCountAd(data.count) // Используем action
  }
