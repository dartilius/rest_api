...
interface IMobileBgTableViewProps {
    data: IBgData[]
    onRowClick: (id: string) => void
}

const MobileBgTableView = ({ data, onRowClick }: IMobileBgTableViewProps) => {
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, p: 1 }}>
            {data.map((row) => {
                ...
                const broadcastType =
                    ORDER_TYPE_BG_CONFIG[row.order_type as keyof typeof ORDER_TYPE_BG_CONFIG] ||
                    'Неизвестный тип'
...
export default MobileBgTableView;