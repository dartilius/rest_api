...
import { NomenclaturesService } from "@/src/services/nomenclatures";
import {
  DaySettings,
...
export default function NomenclatureClientPage({
  id,
  data: initialData,
}: Props) {
  const [data, setData] = useState<NomenclatureInterface | undefined>(
    initialData,
  );
  ...
  useEffect(() => {
    if (!initialData) {
      const fetchNomenclature = async (id: string) => {
        try {
