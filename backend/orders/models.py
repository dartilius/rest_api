...
class BaseOrder(APIBaseObjectModel):
    ...
    БЕССРОЧНЫЕ ЗАКАЗЫ (is_permanent):
        Поле is_permanent добавлено в BgOrder.
    ...
    def get_broadcast_start_local(self):
        ...
        Использует f-string для получения строкового представления,
        которое автоматически отображается без часового пояса.

        Returns:
            str: Строка в формате 'YYYY-MM-DD HH:MM:SS' или None
        """
        if self.broadcast_interval and self.broadcast_interval.lower:
            return f'{self.broadcast_interval.lower}'
        return None
    ...
    def get_broadcast_end_local(self):
        ...
        if self.broadcast_interval and self.broadcast_interval.upper:
            return f'{self.broadcast_interval.upper}'
        return None
