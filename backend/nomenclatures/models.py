...
class TypeOfPlace(models.Model):
    ...
    abbreviation = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Аббревиатура"
    )
    ...
    is_mall = models.BooleanField(
        default=False, verbose_name="Является торговым центром"
    )
...
class DiscountRule(models.Model):
    ...
    days_from = models.PositiveIntegerField(
        verbose_name="Дней (от)", help_text="Включительно"
    )
    ...
    def get_coefficient(cls, nomenclature_id, duration_days: int):
        ...
        """
        rule = (
            cls.objects.filter(nomenclature_id=nomenclature_id)
            .filter(days_from__lte=duration_days)
            .filter(
                models.Q(days_to__gte=duration_days) | models.Q(days_to__isnull=True)
            )
            .order_by("-days_from")
            .first()
        )
...
class Nomenclature(APIBaseObjectModel):
    ...
    external_video_media = models.CharField(
        verbose_name="Видео носители (кол-во внеш.)", null=True, blank=True, default=""
    )
    external_audio_media = models.CharField(
        verbose_name="Аудио носители (кол-во внеш.)", null=True, blank=True, default=""
    )
    internal_video_media = models.CharField(
        verbose_name="Видео носители (кол-во внут.)", null=True, blank=True, default=""
    )
    internal_audio_media = models.CharField(
        verbose_name="Аудио носители (кол-во внут.)", null=True, blank=True, default=""
    )
    ...
    id_rasb = models.CharField(
        null=True, blank=True, verbose_name="Id тачки", default=""
    )

    square = models.CharField(default="", null=True, blank=True, verbose_name="Площадь")

    possibility = models.CharField(
        default="", null=True, blank=True, verbose_name="Проходимость"
    )
    ...
    code1c = models.CharField(
        verbose_name="Код из 1С", max_length=64, blank=True, null=True
    )
    ...
    def make_old_catalog_slug_unique(self, slug):
        ...
        while (
            ...
            .filter(old_catalog_slug=candidate)
            .exists()
        ):
...
class StatusHistory(models.Model):
    ...
    status = models.PositiveSmallIntegerField(choices=STATUSES, verbose_name="Статус")
...
class NomenclatureImage(models.Model):
    ...
    id = models.UUIDField(
        primary_key=True, default=uuid4, editable=False, verbose_name="ИД"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.source and (self._state.adding or not self.source._committed):
...
class NomenclatureVideo(models.Model):
    ...
    type = models.CharField(
        max_length=31, choices=VideoType.choices, verbose_name="Тип видеозаписи"
    )
