...
from counterparties.serializers import (
    ...
    CounterpartyContactInfoSerializer,
)
...
class NomenclatureExamples:
    ...
    UPDATE_REQUEST = OpenApiExample(
        ...
        value={"name": "Новое название", "description": "Новое описание"},
        request_only=True,
    )
...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                ...
                examples=[
                    OpenApiExample(
                        "Онлайн", value=0, description="Только активные номенклатуры"
                    )
                ],
            ),
            OpenApiParameter(
    ...
    partial_update=extend_schema(
        ...
        responses={
            ...
            400: OpenApiResponse(
                ...
                examples=[
                    OpenApiExample(
                        "Запрещенное поле",
                        value={
                            "detail": "Редактирование запрещено для полей: is_active"
                        },
                        response_only=True,
                        status_codes=[400],
                    )
            ...
            404: OpenApiResponse(
                description="Номенклатура не найдена", response=DetailSerializer
            ),
        },
        tags=["Номенклатуры"],
    ),
    destroy=extend_schema(
        ...
        """,
        responses={
            204: OpenApiResponse(description="Номенклатура успешно деактивирована"),
            400: OpenApiResponse(
                ...
                examples=[
                    OpenApiExample(
                        "Уже деактивирована",
                        value={
                            "detail": "Нельзя деактивировать номенклатуру, т.к она уже деактивирована."
                        },
                        response_only=True,
                        status_codes=[400],
                    )
...
class NomenclatureViewSet(SignedMediaNoCacheMixin, viewsets.ModelViewSet):
    ...
    def by_city(self, request, city_slug=None):
        ...
        min_price = (
            queryset.aggregate(min_price=models.Min("pricePerMonth"))["min_price"]
            or 0.0
        )

        serializer = CityNomenclaturesSerializer(
            queryset, many=True, context={"city_name": city.name}
        )
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            base_queryset = NomenclatureSearchService.search(
                query=search_term, for_web=True, limit=5000, use_cache=True
            )
            ...
            tc_ids = set(
                TypeOfPlace.objects.filter(is_mall=True).values_list("id", flat=True)
            )
            ...
            brand_freq = Counter(n.brand_id for n in queryset if n.brand_id is not None)
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: OpenApiResponse(
                description="Неверный параметр", response=DetailSerializer
            ),
        },
    )
    ...
    def tabs(self, request, pk):
        ...
        if not tab:
            raise ValidationError(
                {"q": "Query-параметр 'q' обязателен. Например: ?q=tenants"}
            )
    ...
    def _tenants_tab(self, nomenclature):
        ...
        serializer = CounterpartiesShortSerializer(
            nomenclature.tenants.all(), many=True
        )
    ...
    def _contacts_tab(self, nomenclature):
        ...
        if legal_entity:
            result["legal_entity"] = CounterpartyContactInfoSerializer(
                legal_entity.contacts.all(), many=True
            ).data
        ...
        if nomenclature.responsible_placement_marketing:
            ...
            result["marketing"] = UserContactInfoSerializer(contacts, many=True).data

        if nomenclature.responsible_ad:
            contacts = nomenclature.responsible_ad.contacts_cp.all()
            result["ad"] = UserContactInfoSerializer(contacts, many=True).data
    ...
    def grouped(self, request):
        ...
        qs = filterset.qs.annotate(tenants_count=Count("tenants", distinct=True))
        ...
        if group_by not in GROUP_MAP:
            return Response(
                {"error": "Invalid group param", "allowed": list(GROUP_MAP.keys())},
                status=400,
            )
    ...
    def broadcast(self, request):
        ...
        if (not user.is_authenticated and not self._is_admin(request.user)) or (
            ...
            return Response(
                {"message": "Недостаточно прав."}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        if self._is_admin(request.user):
            ...
            qs = self.get_queryset().filter(
                legalEntity__in=user_counterparties,
                is_active=True,
                legalEntity__broadcast=True,
            )
        else:
            return Response(
                {"message": "Недостаточно прав."}, status=status.HTTP_403_FORBIDDEN
    ...
    def get_versions(self, request):
        ...
        """
        versions = (
            Nomenclature.objects.order_by().values_list("version", flat=True).distinct()
        )
    ...
    @extend_schema(
        ...
        responses={
            200: inline_serializer(
                name="GetIdResponse", fields={"id": serializers.CharField()}
            ),
    ...
    def get_id(self, request):
        ...
        id_rasb = (
            request.query_params.get("id_rasb") or request.data.get("id_rasb")
    ...
    def _is_admin(user):
        ...
        return user.is_authenticated and (
            user.is_admin or user.is_superuser or user.is_manager
        )
