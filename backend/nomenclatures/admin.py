...
3. Кеширование всего queryset для уменьшения количества запросов
...
class NomenclatureAdmin(admin.ModelAdmin):
    ...
    # =========================================================================
    # ОПТИМИЗИРОВАННЫЙ QUERYSET С КЕШИРОВАНИЕМ
    # =========================================================================

    def get_queryset(self, request):
        """
        Оптимизированный запрос для списка номенклатур с кешированием.

        Кешируется весь queryset с предзагруженными данными для ускорения.
        """
        ...
        if queryset is None:
            queryset = (
                ...
                .select_related(
                    "owner",           # Владелец (custom_user)
                    "availability",    # Статус доступности
                    "brand",           # Бренд
                    "legalEntity",     # Юр. лицо (counterparties)
                    "responsible_radio",
                    "responsible_ad",
                    "responsible_technic",
    ...
    def owner_name(self, obj):
        """Данные загружены через select_related('owner')."""
        if not obj.owner:
            return "-"
    ...
    def status_display(self, obj):
        """Данные загружены через select_related('availability')."""
        try:
    ...
    def brand_name(self, obj):
        """Данные загружены через select_related('brand')."""
    ...
    def legal_entity_name(self, obj):
        """Данные загружены через select_related('legalEntity')."""
    ...
    def tenants_count_display(self, obj):
        """Данные загружены через annotate(tenants_count=Count(...))."""
